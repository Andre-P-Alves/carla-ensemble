        ...
        for route_idx in range(route_start, route_start + n_routes):
            for ep_idx in range(ep_start, ep_start + n_eps):
                ...
                for step_idx in range(traj_length):
                    random_sampler = random.randint(0, traj_length)
                    self.get_idx.append((route_idx, ep_idx, random_sampler))
                    state_dict = {}
                    for state_key in route_df.columns:
                        state_dict[state_key] = route_df.iloc[random_sampler][state_key]
                    self.trajs_states.append(state_dict)
                    self.trajs_actions.append(th.Tensor(route_df.iloc[random_sampler]['actions']))
