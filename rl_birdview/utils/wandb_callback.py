# modified from https://github.com/zhejz/carla-roach/blob/main/agents/rl_birdview/utils/wandb_callback.py
...
class WandbCallback(BaseCallback):
    def __init__(self, vec_env, wandb_run_id=None):
    ...
    def _init_callback(self):
        ...
        for i in range(self.vec_env.num_envs):
            self.vec_env.set_attr('n_epoch', self.model.i_update, indices=i)
            self.vec_env.set_attr('num_timesteps', self.model.num_timesteps, indices=i)
    ...
    def _on_training_end(self) -> None:
        ...
        wandb.log(self.model.discriminator.disc_debug, step=self.model.num_timesteps)

        # evaluate and save checkpoint
        if (self.model.num_timesteps % self._eval_step) < self.model.n_steps_total :
            self._last_time_eval = self.model.num_timesteps
            # evaluate
            eval_video_path = (self._video_path / f'eval_{self.model.num_timesteps}.mp4').as_posix()
            avg_ep_stat, avg_route_completion, ep_events = self.evaluate_policy(self.vec_env, self.model.policy, eval_video_path)
            # log to wandb
            ...
            wandb.log(avg_route_completion, step=self.model.num_timesteps)
            # save events
        ...
        ckpt_path = (self._ckpt_dir / f'ckpt_latest.pth').as_posix()
        self.model.save(ckpt_path)
        wandb.save(f'./{ckpt_path}')
        self.n_epoch += 1
    ...
    def _on_rollout_end(self):
        ...
        wandb.log(avg_ep_stat, step=self.model.num_timesteps)
        avg_route_completion = self.get_avg_route_completion(self.model.route_completion_buffer, prefix='rollout/')
        wandb.log(avg_route_completion, step=self.model.num_timesteps)
    ...
    def evaluate_policy(env, policy, video_path, min_eval_steps=3000):
        ...
        while n_step < min_eval_steps or not np.all(env_done):
            actions, values, log_probs, mu, sigma, _, fake_birdview = policy.forward(obs, deterministic=True, clip_action=True)
        ...
        avg_route_completion = WandbCallback.get_avg_route_completion(route_completion_buffer, prefix='eval/')
